...
user='www-data'
group='www-data'
if ! [ -e VERSION -a -e index.php ]; then
    echo >&2 "Calfbbs not found in $PWD - copying now..."
    if [ "$(ls -A)" ]; then
    ...
    tar --create \
        --file - \
        --one-file-system \
        --directory /usr/src/calfbbs \
        --owner "$user" --group "$group" \
        . | tar --extract --file -
    echo >&2 "Complete! Calfbbs has been successfully copied to $PWD"
fi
